...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->num_rows === 1) {
            $stmt->bind_result($userId, $hashed_password); // Fetch ID as well
            $stmt->fetch();
            ...
            if (password_verify($password, $hashed_password)) {
                ...
                $updateStmt = $conn->prepare("UPDATE users SET IsLoggedIn = TRUE WHERE username = ? OR email = ?");
                $updateStmt->bind_param("ss", $usernameOrEmail, $usernameOrEmail);
                $updateStmt->execute();
                $updateStmt->close();
                ...
                // Set up JavaScript to store values in local storage
                echo "<script>
                        localStorage.setItem('userId', '$userId');
                        localStorage.setItem('username', '$usernameOrEmail');
                        window.location.href = '../Menu/Dagwood_Menu/dagwood_menu.php'; // Redirect after setting local storage
                      </script>";
